pipeline {
    ...
    environment {
        REGISTRY = 'harbor.civiclink.internal'
    }
    stages {
        ...
        stage('Sign')  { steps { sh 'cosign sign --yes ${REGISTRY}/civiclink/${SERVICE}:${GIT_COMMIT}' } }
        stage('Push')  { steps { sh 'make push' } }
    }
    post {
        failure { slackSend channel: '#civiclink-ci', message: "Build failed: ${env.BUILD_URL}" }
    }
}