...
spark = SparkSession.builder.appName("civiclink-segment").getOrCreate()
df = spark.read.format("iceberg").load("civiclink_mart.user_features")
...
preds.select("user_id","prediction").writeTo("civiclink_mart.user_segments").using("iceberg").createOrReplace()
