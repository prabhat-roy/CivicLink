# Multi-Cloud Architecture — CivicLink
...
complete CivicLink-ready cluster on that single cloud — no cross-cloud
dependencies, no shared remote state, no assumed global resource names.

## Independence rules

1. **Per-cloud state** — each cloud uses its native remote backend (S3 + DynamoDB,
   GCS, Azure Storage). State is never shared.
2. **Per-cloud naming** — every resource is namespaced `civiclink-<env>` and lives
   inside that cloud's account/project/subscription.
3. **Cloud-agnostic Helm charts** — `helm/charts/<service>/values.yaml` references
...
4. **Cloud-equivalent capabilities** — every primitive used at the application
...
deployed inside the cluster on every cloud — cloud-native logging is only used
...
- Asymmetric cross-cloud replication for the document vault (S3 ↔ GCS ↔ Blob via
