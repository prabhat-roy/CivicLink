...
deny[msg] if {
    ...
    not startswith(container.image, "harbor.civiclink.internal/")
    msg := sprintf("image %q must be from harbor.civiclink.internal", [container.image])
}
