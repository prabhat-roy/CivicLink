    ...
    stages {
        stage('Configure kubeconfig') {
            steps {
                script {
                    if (params.CLOUD == 'aws') {
                        sh 'aws eks update-kubeconfig --name civiclink-${ENV} --region us-east-1'
                    } else if (params.CLOUD == 'gcp') {
                        sh 'gcloud container clusters get-credentials civiclink-${ENV} --region europe-west4 --project civiclink-${ENV}'
                    } else {
                        sh 'az aks get-credentials --resource-group civiclink-${ENV} --name civiclink-${ENV}'
                    }
                }
            }
        }
        ...
        stage('Verify') {
            steps {
                sh 'kubectl get pods -n civiclink'
